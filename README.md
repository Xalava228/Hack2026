...
Сервис для автоматической генерации презентаций по текстовому запросу — аналог
[gamma.app](https://gamma.app), работающий на API `ai.rt.ru`
(LLM Qwen 2.5 + Yandex ART + Stable Diffusion).

## Что умеет

- Принимает промпт в духе «История развития ИИ от Тьюринга до GPT-5»
- Сам выбирает структуру презентации (LLM возвращает JSON-план: тип слайда,
  заголовки, буллеты, цветовая палитра)
...
- Красивый интерфейс в стиле gamma.app: форма → прогресс по шагам → скачивание
